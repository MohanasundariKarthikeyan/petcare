...
$orders_query = $db->prepare("SELECT * FROM orders7 WHERE username = ? ORDER BY created_at DESC");
$orders_query->bind_param("s", $username);
        ...
        <?php if ($orders->num_rows > 0): ?>
            <?php while ($order = $orders->fetch_assoc()): ?>
                ...
                <div class="order-card">
                    ...
                    <!-- Display username if available in the order data -->
                    <?php if (!empty($order['username'])): ?>
                    <div class="username-display">
                        <i class="fas fa-user"></i>
                        Order for: <?php echo htmlspecialchars($order['username']); ?>
                    </div>
                    <?php endif; ?>
